var pokiDebug = false;
    ...
    function domContentLoaded() {
      ...
      let savedLevel = 1;
      try {
        const stored = localStorage.getItem('drive_mad_current_level');
        if (stored !== null) {
        ...
        console.warn("Failed to read level from localStorage:", e);
      }

      updateUserLevel(savedLevel);
      let canvas = document.getElementById('canvas');
    ...
    function updateUserLevel(level) {
      const levelEl = document.getElementById('user_level');
      if (levelEl) {
